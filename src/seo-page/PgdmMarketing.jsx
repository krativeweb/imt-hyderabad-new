...
export default function PGDMMarketingSection() {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      try {
        const res = await fetch(`${API_URL}/api/pgdm-marketing`);
        const json = await res.json();
        setPageData(json[0]);
      } catch (err) {
        console.error("Failed to load PGDM Marketing page", err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <section id="pgdm-tabs" className="py-5 faculty-tabs">
        <div className="container" data-aos="fade-up" data-aos-delay="200">
          ...
          <p className="text-center mb-5" style={{ color: "#08317a" }}>
             <div dangerouslySetInnerHTML={{ __html: pageData.pgdm_marketing }} />
          </p>

          {/* -------- TAB BUTTONS (STATIC) -------- */}
          <ul className="nav nav-pills justify-content-center mb-4 flex-wrap">
            <li className="nav-item m-1">
              <button className="nav-link active fw-bold" data-bs-toggle="pill" data-bs-target="#curriculum">
                CURRICULUM
              </button>
            </li>
            <li className="nav-item m-1">
              <button className="nav-link fw-bold" data-bs-toggle="pill" data-bs-target="#keyfeatures">
                KEY FEATURES
              </button>
            </li>
            <li className="nav-item m-1">
              <button className="nav-link fw-bold" data-bs-toggle="pill" data-bs-target="#programoutcome">
                PROGRAM OUTCOME
              </button>
            </li>
            <li className="nav-item m-1">
              <button className="nav-link fw-bold" data-bs-toggle="pill" data-bs-target="#pedagogy">
                PEDAGOGY
              </button>
            </li>
            <li className="nav-item m-1">
              <button className="nav-link fw-bold" data-bs-toggle="pill" data-bs-target="#career">
            ...
            <div className="card-body">
              <div className="tab-content">
                ...
                <div className="tab-pane fade show active" id="curriculum">
                  <div dangerouslySetInnerHTML={{ __html: pageData.curriculum }} />
                </div>

                <div className="tab-pane fade" id="keyfeatures">
                  <div dangerouslySetInnerHTML={{ __html: pageData.key_features }} />
                </div>

                <div className="tab-pane fade" id="programoutcome">
                  <div dangerouslySetInnerHTML={{ __html: pageData.program_outcome }} />
                </div>

                <div className="tab-pane fade" id="pedagogy">
                  <div dangerouslySetInnerHTML={{ __html: pageData.pedagogy }} />
                </div>

                <div className="tab-pane fade" id="career">
                  <div dangerouslySetInnerHTML={{ __html: pageData.career_opportunities }} />
                </div>

                <div className="tab-pane fade" id="competency">
                  <div dangerouslySetInnerHTML={{ __html: pageData.competency_goal }} />
                </div>
