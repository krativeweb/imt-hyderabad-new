    ...
    const fetchPage = async () => {
      try {
        const res = await fetch(
          `${API_URL}/api/pgdm-logistics-supply-chain`
        );
        const json = await res.json();
        setPageData(json[0]);
      } catch (err) {
        console.error("Failed to load PGDM LSCM page", err);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="container" data-aos="fade-up" data-aos-delay="200">
         ...
         PGDM (Logistics & Supply Chain Management)
          ...
          <p className="text-center mb-5" style={{ color: "#08317a" }}>
            <div
                    dangerouslySetInnerHTML={{ __html: pageData.pgdm_logistics_supply_chain }}
                  />
          ...
          <ul className="nav nav-pills justify-content-center mb-4 flex-wrap">
            {[
              ...
              <li className="nav-item m-1" key={id}>
                <button
                  className={`nav-link fw-bold ${i === 0 ? "active" : ""}`}
                  data-bs-toggle="pill"
                  data-bs-target={`#${id}`}
                >
                  {label}
